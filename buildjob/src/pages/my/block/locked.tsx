...
export default function Locked() {
  ...
  async function getBanDetail() {
    try {
      let {data : {list}} = await api.get(eps["GET_BAN"](2, 0, 10));
      setDetailList(list)
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <>
      <header className={styles.header}>
        ...
        <h1 className={styles.key}>계정이 영구정지 되었습니다.</h1>

        <p className={styles.value}>
          직업안정법 제 25조 및 시행령 28조에 따라
          <br />
          사용자 간 불미스러운 일 발생으로
          <br />
      ...
      <main className={styles.locked}>
        <section className={`${styles.infoSec} ${styles.contSec}`}>
          <h1 className={styles.contTitle}>계정정지 사유 안내</h1>
          ...
          <article className={styles.contArea}>
            ...
            <div className={styles.actionCont}>
              <button
                ...
                onClick={() => navigate("/mypage/block/penalty", { state: {type: 2} })}
              >
                위약금 납부하기
              </button>
