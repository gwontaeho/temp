...
export const FormControl = forwardRef((props, ref) => {
    const { type, invalid, getValues, edit, button, leftText, leftButton, rightText, rightButton, size = "full", ...inputProps } = props;

    const isEditFalse = edit === false;
    const hasLeftButton = Boolean(leftButton && !isEditFalse);
    const hasRightButton = Boolean(rightButton && !isEditFalse);
    ...
    switch (type) {
        ...
        case "file":
            ...
            break;
        case "between":
            control = <InputBetween ref={ref} edit={edit} hasRightButton={hasRightButton} hasLeftButton={hasLeftButton} {...inputProps} />;
            break;
        default:
            control = <InputText ref={ref} {...inputProps} />;
            break;
    ...
    return (
        <div className={classNames("space-y-1", SIZES[size])}>
            ...
            <div
                ref={controlRef}
                className={classNames("flex w-full", { "[&_.input]:rounded-r-none": hasRightButton, "[&_.input]:rounded-l-none": hasLeftButton })}
            >
                {hasLeftButton && (
                    <button type="button" className="bg-bg flex items-center space-x-1 px-2 py-1 border-y border-l rounded-l" onClick={leftButton.onClick}>
                        {leftButton.icon && <Icon icon={leftButton.icon} size="sm" />}
                ...
                {hasRightButton && (
                    <button
                        type="button"
                        className="bg-bg flex items-center min-w-fit space-x-1 px-2 py-1 border-y border-r rounded-r"
                        onClick={rightButton.onClick}
                    >
                        {rightButton.icon && <Icon icon={rightButton.icon} size="sm" />}
...
const InputDate = forwardRef((props, ref) => {
    ...
    return (
        <div className="relative w-full [&>div]:w-full">
            <Icon icon="calendar" size="sm" className="absolute left-1 top-1/2 -translate-y-1/2 z-10" />
            {control ? (
                <Controller
                    name={name}
                    control={control}
                    ...
                    render={({ field: { onChange, value } }) => (
                        <ReactDatePicker {...rest} selected={value} onChange={onChange} className="input pl-5" popperProps={{ strategy: "fixed" }} />
                    )}
                />
            ) : (
                <ReactDatePicker {...rest} selected={selected} onChange={setSelected} className="input pl-5" popperProps={{ strategy: "fixed" }} />
            )}
        </div>
    );
});
...
const InputTime = forwardRef((props, ref) => {
    ...
    return (
        <div className="w-full [&>div]:w-full">
            <Icon icon="clock" size="sm" className="absolute left-1 top-1/2 -translate-y-1/2 z-10" />
            {control ? (
                <Controller
                    name={name}
                    control={control}
...
const InputBetween = forwardRef((props, ref) => {
    ...
    const Begin = () => {
        ...
        return <FormControl {...schema} edit={edit} className={classNames("rounded-r-none")} />;
    };

    const End = () => {
        const schema = end[1];
        return <FormControl {...schema} edit={edit} className={classNames("rounded-l-none", { "rounded-r-none": options })} />;
    };
    ...
    return (
        <div className={classNames("w-full flex items-center")}>
            ...
            <div className={classNames("flex items-center justify-center h-7 w-5", { "bg-header border-y": !isEditFalse })}>-</div>
            <div className={classNames({ "flex-1": !isEditFalse })}>
                <End />
            </div>
            {!isEditFalse && options && (
